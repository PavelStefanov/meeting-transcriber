...
e = d['engines']
assert e['active'] in ('whisperKit', 'parakeet', 'whisperCpp'), f'bad active: {e[\"active\"]!r}'
assert 'modelVariant' in e['whisperKit']
assert 'customVocabularyPath' in e['parakeet']
# whisper.cpp reports whether its 2.9 GB model is on disk, which is what an
# A/B/C comparison needs to check before it trusts a run.
assert 'modelDownloaded' in e['whisperCpp']
assert 'modelState' in e['whisperCpp']
